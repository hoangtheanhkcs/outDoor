...
class VideoLibraryCollectionViewCell: UICollectionViewCell {
    ...
    func setupVide0(item: AssetItem) {
        
        
        guard item.assetType == .video else {return}
        ...
        if item.asset == nil {
            ...
            imageVideo = nil
            recoringImageView.image = UIImage(named: Constants.Images.takeNewVideo)
            recordingLable.setupAutolocalization(withKey: Constants.Strings.takeNewVideo, keyPath: "text")
        }else if item.asset != nil {
            imageVideo = item.asset
            videoImage.image = item.image
            let duration = String.duration(from: item.asset!.duration)
